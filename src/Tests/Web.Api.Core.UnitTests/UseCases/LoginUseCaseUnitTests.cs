...
namespace Web.Api.Core.UnitTests.UseCases
{
    public class LoginUseCaseUnitTests
    {
        ...
        public async void Handle_GivenValidCredentials_ShouldSucceed()
        {
            ...
            mockUserRepository.Setup(repo => repo.FindByEmail(It.IsAny<string>())).ReturnsAsync(new User("","", Guid.Parse(null), ""));
            ...
            mockJwtFactory.Setup(factory => factory.GenerateEncodedToken(It.IsAny<Guid>(), It.IsAny<string>())).ReturnsAsync(new AccessToken("", 0));
        ...
        public async void Handle_GivenUnknownCredentials_ShouldFail()
        {
            // arrange
            var mockUserRepository = new Mock<IUserRepository>();
            mockUserRepository.Setup(repo => repo.FindByEmail(It.IsAny<string>())).ReturnsAsync((User)null);
